...
export default function Signup() {
  ...
  //   const response = await fetch("http://localhost:5000/tn/users", {
  //     method: 'POST',
  //     headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch("http://localhost:5000/tn/user/new", {
      // credentials: 'include',
      ...
      body: JSON.stringify({ name: credentials.name, email: credentials.email, password: credentials.password, location: credentials.geolocation,orders:[] })
